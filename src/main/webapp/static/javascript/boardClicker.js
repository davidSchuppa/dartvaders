...
class Game {
     ...
     */
    switchPlayers() {
        this.currentPlayer = this.player[this.currentPlayer.id === 1 ? 'two' : 'one'];
    }
    ...
    addDart(id) {
        ...
        // [2]
        this.currentPlayer.currentScore = this.currentPlayer.currentThrow.calculateCurrentScore(this.currentPlayer.currentScore, id);
...
angular.module('app', [])
    .controller('gameCtrl', $scope => {
         ...
         */
        $scope.dartboardSections = document.querySelectorAll('#dartboard #areas g path, #dartboard #areas g circle')
        ...
        $scope.boardClickHandler = event => {
            ...
            // [2]
            if ($scope.game.player.one.currentScore === 0 || $scope.game.player.two.currentScore === 0) {
                $scope
                    .dartboardSections
                    .forEach(p => p.removeEventListener('click', $scope.boardClickHandler));
            }
        ...
        $scope
            .dartboardSections
            .forEach(p => p.addEventListener('click', $scope.boardClickHandler));
    });